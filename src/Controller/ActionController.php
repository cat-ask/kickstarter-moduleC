...
class ActionController{
    ...
    function investorsAdd(){
        ...
        $sql = "INSERT INTO investors(`email`,`pay`,`datetime`,`fund_id`) VALUES(?,?,?,?)";
        DB::query($sql,[user()->user_email,$viewMoney,date('Y-m-d H:i:s'),$viewFundId]);

        $fund = DB::find("funds",$viewFundId);
        $current = (int)$fund->current + (int)$viewMoney;
        $achieve = round($current / ((int)$fund->total / 100));
        ...
        $owner = DB::find("user",$viewFundOwnerId);
        $owner_money = (int)$owner->money + (int)$viewMoney;
        DB::query("UPDATE user SET `money` = ? WHERE id = ?",[$owner_money,$viewFundOwnerId]);
    ...
    function userPageList(){
        ...
        $sql = "SELECT F.name, F.content FROM funds AS F, investors AS I WHERE I.fund_id = F.id AND I.email = ? AND F.status = ?";
        $user_end_fundList = DB::fetchAll($sql,[$user->user_email,"business"]);
    
        echo json_encode(compact("user_register_fundList","user_fundList","user_end_fundList"));
    }
    ...
    function fundListLoad(){
        ...
        foreach($result as $fund){
            ...
            $fund->current = (int)$fund->current;
            $fund->achieve = (int)$fund->achieve;
